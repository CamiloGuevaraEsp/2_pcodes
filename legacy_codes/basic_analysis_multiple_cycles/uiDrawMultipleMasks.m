...
for(ri = 1:numRegions),
    ...
    %hold on
    %RGB = insertText(maskInProgress,position,value)
    %imshow(RGB);
%     display('.');
end;
...
for i = 1:numRegions
    text(maskmat.position(i,1),maskmat.position(i,2),num2str(maskmat.value(i)), 'Color', 'black','FontSize',24);
end

figure(3)
imagesc(maskmat.avgImg,[0 100]); 
for i = 1:numRegions
