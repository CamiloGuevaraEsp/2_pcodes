...
%Notes for 20230331
%Change the definition of name so it can iteratwe through cycle...I dont
%know what that happen If I checked fastest acquisition
%Change medianfiltersize 
...
if(nargin==0),
    ...
    TSeriesFolder =  '20230614_pdf_chrimsom_dfsb_asap_activation_only_zt4_TSeries-015';
    num_slices = 6; 
...
%Use this if the tif files iterate through cycles intead of tif number
tiffList = dir(['*.tif']);
display(TImgRoot);
...
cyclesToIterate = maxFrame2read/num_slices;

% regionPropsArray = cell(maxFrame2read,1);

for i = 1:cyclesToIterate
    for j = 1:num_slices
    
    imgName = [TSeriesFolder '_Cycle' num2str(i,'%05.0f') '_Ch2_' num2str(j, '%06.0f') '.ome.tif']       
...
% 
% for(ti = 1:2:maxFrame2read),
%    % if(mod(ti,15)>7),
% %         display(ti);
% %Use this when it iterates through cycle instead of folder 
%  imgName = [TSeriesFolder '_Cycle' num2str(ti,'%05.0f') '_Ch2_000001'  '.ome.tif']       
%  %imgName = [TImgRoot num2str(ti,'%06.0f') '.ome.tif']
% %         try,
%             rawImg = double(medfilt2(imread(imgName),[medfiltSize medfiltSize]));
%             %     rawImg_doubleVec =double(rawImg(:));
