...
voltageNameLabel = '<MarkPoints category="Vishnu PDF stim mark" name="';
% sequenceStartLabel = '<Sequence type="TSeries ZSeries Element" cycle="';
dateTimeLabel='<PVScan version="5.8.64.400" date="';
...
while(~feof(fID) && voltIndex<=cycleRankToReturn),
    ...
    elseif(~isempty(strfind(line,voltageNameLabel)) && voltIndex<=cycleRankToReturn),
        %If (voltIndex<cycleRankToReturn), stackNum gets read even for stacks collected in the next voltageNum.
        display(['Reading voltageName when voltnum=' num2str(voltIndex) ', Line: ' line]);
%         
%         VolStartIndex = strfind(line,voltageNameLabel);
%         endOfLine = line((VolStartIndex+numel(voltageNameLabel)):end);
%         quoteIndices = strfind(endOfLine,'"');
%         VoltString = endOfLine(1:(quoteIndices(1))-1);
%         startTime_dateNum = datenum(dateTimeString);
        ...
        if(voltIndex<cycleRankToReturn), 
            %If (voltIndex<cycleRankToReturn), second voltageName information gets saved.
