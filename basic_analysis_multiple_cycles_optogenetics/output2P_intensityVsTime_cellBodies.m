...
%By Cynthia T. Hsu and Camilo Guevara

function [timeIntensityMat, lastBaselineIndex, voltOutTimes] = output2P_intensityVsTime_cellBodies(varargin)

if (nargin==0),
    rootdir = 'D:\2p_Imaging\Optogenetic_experiments\pdf_activation_dfsb_recording\20230614_pdf_chrimsom_dfsb_asap_activation_only_zt4'
    signalMatname = '20230614_pdf_chrimsom_dfsb_asap_activation_only_zt4_TSeries-015_Cycle00001_Ch2__userDrawnMask__Intensities_3'; %strrep(TProjMask, '.mat','Intensities.mat');
    cycleRankNum = 3; %Set to 0 if there is no voltage being used, otherwise set to 1.
    % Voltage values are hardcoded into the readTimeFromXML script.
    xmlName = '20230614_pdf_chrimsom_dfsb_asap_activation_only_zt4_TSeries-015.xml';
    numSlices = 6; 
    ...
    plotSignal = 4;
    t0_inSeconds=NaN;
    %5*60*60; %20*60; %Set to NaN if t0_inSeconds should be extracted from shutter information.
...
%For the green interp error, it happenend once because the end of the
%regions was an empty cell, resulting in a regions size of zero, uncomment
%the next line and comment the following one to solve the error

%for (ti = 1:size(regions,1)-1)
...
for(si = 1:numRegions)
    ...
    timeIntensityMat(:,2*si+1) = greenInterp;
...
timeRelativeToStimulus = timeIntensityMat(:,1)-timeIntensityMat(firstShutteredStackIndex,1);
   ...
   for(si=1:size(shutterTimes)),
        minIntensityVal = min(timeIntensityMat(:,3));
        plot([shutterTimes(si,1) shutterTimes(si,2)]-timeIntensityMat(firstShutteredStackIndex,1),[0 1]*min(greenInterp),'r','LineWidth',4); hold on;
%         display('si')
    end;
...
% figure(2);
% 
% %If you get an error that tracks to this line please go to line 64
...
%     for(si=1:size(shutterTimes)),
%         %minIntensityVal = min(timeIntensityMat(:,3));
% %         plot([shutterTimes(si,1) shutterTimes(si,2)]-timeIntensityMat(firstShutteredStackIndex,1),[0 1]*min(normalizedSignal),'r','LineWidth',4); hold on;
% %         display('si')
%     end;
% end;
...
save ('shutterTimes_3.mat','shutterTimes');