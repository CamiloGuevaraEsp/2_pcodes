...
function [stackTimes, shutterTimes, lastBaselineIndex, period, varargout] = readTimeFromXML(varargin)
...
period = '<PVStateValue key="framePeriod" value="';
...
while(~feof(fID) && voltIndex<=cycleRankToReturn),
    ...
    %recordingPeriod = strfind(line, period);
    %period = strfind
